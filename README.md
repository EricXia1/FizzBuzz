...
1. Pythons `and` is equivalent to Javas `&&`
2. The `for` loop seems to be similar to a python `while` loop where it begins an incrementing variable at 1 and increases it by 1 until it reaches 100
3. All of the code is under a class and a method as opposed to Python allowing us to directly code without any other requirements
4. Almost every line ends with a semicolon
5. The console print command is much longer than Pythons
